...
class ArchivedAmbulanceReportsScreen extends StatelessWidget {
  ArchivedAmbulanceReportsScreen({Key? key,}) : super(key: key);
            ...
            fallback: (context) {
              return ConditionalBuilder(
                ...
                fallback: (context)=> ConditionalBuilder(
                    condition: cubit.ambulanceReports.isEmpty,
                    builder: (context)=> const EmptyScreen(text: 'There are no ambulance reports yet'),
                    fallback: (context) {
                      return GridView.count(
                        ...
                        children: List.generate(
                          cubit.ambulanceReports.length,
                              (index) => ArchivedReportCart(reportModel: cubit.ambulanceReports[index]),
                        ),
                      );
                    }
                ),
